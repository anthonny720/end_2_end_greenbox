...
const TableKPIGoldenberry = ({update,params}) => {
    const data = useSelector(state => state.Operations.kpi);
    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(get_kpi('goldenberry',params))
    }, [params])


    const columns = ['', 'Fecha', 'Semana', 'Lotes', 'Proyección', 'Ingreso', 'Cumplimiento', 'Objetivo', 'Mad. 1', 'Mad. 2', 'Mad. 3', 'Hongos y fermentado', 'Verde', 'Aplastado', 'Rajado', 'Precio', 'Objetivo'];
                ...
                <table className="min-w-full bg-white dark:bg-gray-800" >
                    ...
                    {data && data.map((item, index) => {
                        return (<tr className="h-12 border-gray-300 dark:border-gray-200 border-b " key={index}>
                            ...
                            <td className="text-sm whitespace-no-wrap text-gray-800 leading-4 text-center font-light ">{Humanize.formatNumber(item?.info?.maduration_2, 2)}</td>
                            <td className="text-sm whitespace-no-wrap text-gray-800 leading-4 text-center font-light ">{Humanize.formatNumber(item?.info?.maduration_3, 2)}</td>
                            <td className="text-sm whitespace-no-wrap text-gray-800 leading-4 text-center font-light ">{Humanize.formatNumber(item?.info?.mushroom, 2)}</td>
                            <td className="text-sm whitespace-no-wrap text-gray-800 leading-4 text-center font-light ">{Humanize.formatNumber(item?.info?.green, 2)}</td>
                            <td className="text-sm whitespace-no-wrap text-gray-800 leading-4 text-center font-light ">{Humanize.formatNumber(item?.info?.cracked, 2)}</td>
                            <td className="text-sm whitespace-no-wrap text-gray-800 leading-4 text-center font-light ">{Humanize.formatNumber(item?.info?.crushed, 2)}</td>
...
export default TableKPIGoldenberry;
