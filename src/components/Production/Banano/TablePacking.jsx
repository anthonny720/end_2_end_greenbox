...
const TablePacking = ({update,reference}) => {
    ...
    const columns = ['Lote', 'Semana', 'Fecha', 'Kg procesados', 'Kg Total', 'Lote coins','Kg coins', '% Rend', 'Lote slices','Kg slices', '% Rend', 'Kg recuperable', 'Venta local', 'Calidad', '% Rendimiento procesado', '% Rendimiento neto']
                ...
                <table className="min-w-full bg-white dark:bg-gray-800" ref={reference}>
                    ...
                    {data && map(data, (item, index) => {
                        return (<tr className="h-12 border-gray-300 dark:border-gray-200 border-b " key={index}>
                            <td title={`Fecha: ${item?.date} \n Kg: ${item?.paid_kg}`} onClick={() => update(item,'packing')}
                            ...
                            <td className="text-sm bg-white px-6 whitespace-no-wrap text-gray-800 font-bold leading-4 text-center ">{item?.lot_cut_coins}</td>
                            <td className="text-sm bg-white px-6 whitespace-no-wrap text-gray-800 font-bold leading-4 text-center ">{Humanize.formatNumber(item?.pt_cut_coins, 2)}</td>
                            <td className="text-sm bg-white px-6 whitespace-no-wrap text-gray-800 font-normal leading-4 text-center ">{Humanize.formatNumber(item?.performance_coins, 2)}</td>
                            <td className="text-sm bg-white px-6 whitespace-no-wrap text-gray-800 font-bold leading-4 text-center ">{item?.lot_cut_slices}</td>
                            <td className="text-sm bg-white px-6 whitespace-no-wrap text-gray-800 font-bold leading-4 text-center ">{Humanize.formatNumber(item?.pt_cut_slices, 2)}</td>
                            <td className="text-sm bg-white px-6 whitespace-no-wrap text-gray-800 font-normal leading-4 text-center ">{Humanize.formatNumber(item?.performance_slices, 2)}</td>
